/*
        ...
        Date: 12/11/19
        Class: CS1121 L01
        Description: The program takes a file with blanks in the text and allows the user to fill in the blanks. Each
 ...
 */
    ...
    public  ArrayList<String> predictionFill(File theFile)
    {
        TextPredictor  run = new TextPredictor();
        Scanner scanTxt = new Scanner(System.in);
        ArrayList<String> textTemplate = run.loadMessage(theFile);
        for(int i = 0;i<textTemplate.size();i++)  // runs through the array list to find the prompts
        {
            if(textTemplate.get(i).contains("[NOUN]"))
            {
                System.out.println("Please input a noun");
                String txt = scanTxt.nextLine();
                textTemplate.set(i,txt);
            }
            else if(textTemplate.get(i).contains("[PROPER-NOUN]")) // checking for the specified prompt
            {
                System.out.println("Please input a proper noun");
                String txt = scanTxt.nextLine();
                textTemplate.set(i,txt);
            }
            else if(textTemplate.get(i).contains("[ADJECTIVE]"))
            {
                System.out.println("Please input an adjective");
                String txt = scanTxt.nextLine();
                textTemplate.set(i,txt);
            }
            else if(textTemplate.get(i).contains("[VERB]"))
            {
                System.out.println("Please input a verb");
                String txt = scanTxt.nextLine();
                textTemplate.set(i,txt);
            }
            else if(textTemplate.get(i).contains("[SPORT]"))
            {
                System.out.println("Please input a sport");
                String txt = scanTxt.nextLine();
                textTemplate.set(i,txt);
            }
            else if(textTemplate.get(i).contains("[FOOD]"))
            {
    ...
    {
        TextPredictor  run = new TextPredictor();
        ArrayList<String> textTemplate = run.predictionFill(theFile);
        ...
        try {
            ...
            for (int i = 0; i < textTemplate.size() ; i++)
            {
                ...
                else if (text.length() < 40 && i == textTemplate.size()-1) // Printing out if the last line is too short to
